...
def subplot(ax, maps, metric, models):
    ...
    for i, model in enumerate(models):
        ...
        ax.plot(range(6), means, '-o', label=model, color=colors[i])
        ax.fill_between(range(6), means - stds, means + stds, alpha=0.2, color=colors[i])
...
def graph_reconstruction_results(celebs, models_list):
    ...
    plt.savefig(os.path.join('/playpen-nas-ssd/awang/mystyle_original/out/reconstruction_at_5_plot.png'))
