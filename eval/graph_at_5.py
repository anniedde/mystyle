...
def subplot(ax, map, metric, times, models):
...
def graph_reconstruction_results(celeb, device, models_list=None):
    ...
    subplot(axs[0, 0], lpips_map, 'LPIPS', times, models_list)
    subplot(axs[0, 1], psnr_map, 'PSNR', times, models_list)
    subplot(axs[1, 0], dists_map, 'DISTS', times, models_list)
    subplot(axs[1, 1], id_error_map, 'ID Error', times, models_list)
