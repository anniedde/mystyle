...
def generate(celeb, experiment, latent_dir, output_path, model, device):
    if os.path.isdir(os.path.join(output_path, 'images')) and len(os.listdir(os.path.join(output_path, 'images'))) >= 20:
...
if __name__ == '__main__':
    ...
    for model in models:
        ...
        latest_vid = model + 1 if args.experiment != 'upper_bound' else 10
        for video in range(0, latest_vid):
        ...
        for video in range(0, latest_vid):
            ...
            latent_dir = os.path.join(celeb_dir, str(video), 'train', 'anchors')
            out_dir = os.path.join('/playpen-nas-ssd/awang/mystyle_original/out', args.celeb, 'synthesis', args.experiment, str(model), str(video))

            generate(args.celeb, args.experiment, latent_dir, out_dir, model, args.device)
            evaluate_synthesis(args.celeb, video, args.experiment, model)